...
########## Sets ##########
  ...
  set: return_detail {
    ...
    # Book-specific measures
    measure: books_sold_today {
      label: "Books Sold Today"
      description: "Number of books sold today"
      type: count_distinct
      sql: ${id} ;;
      filters: [created_date: "1 days"]
    }

    measure: books_per_transaction {
      label: "Books per Transaction"
      description: "Average number of books per customer transaction"
      type: average
      sql: CASE WHEN ${order_id} IS NOT NULL THEN 1.0 ELSE 0 END ;;
      value_format_name: decimal_1
    }

    measure: total_sale_price_idr {
      label: "Total Sales (IDR)"
      description: "Total sales revenue in Indonesian Rupiah"
      type: sum
      sql: ${sale_price} * 15000 ;; # Approximate USD to IDR conversion
      value_format: "\"Rp \"#,##0"
    }

    measure: average_sale_price_idr {
      label: "Average Book Price (IDR)"
      type: average
      sql: ${sale_price} * 15000 ;;
      value_format: "\"Rp \"#,##0"
    }

    measure: fiction_sales_percentage {
      label: "Fiction Sales %"
      description: "Percentage of sales from Fiction books"
      type: number
      sql:
      100.0 *
      SUM(CASE WHEN ${products.category} = 'Tops & Tees' THEN ${sale_price} ELSE 0 END) /
      NULLIF(SUM(${sale_price}), 0)
    ;;
      value_format_name: percent_1
    }

    measure: childrens_books_percentage {
      label: "Children's Books Sales %"
      type: number
      sql:
      100.0 *
      SUM(CASE WHEN ${products.category} = 'Shorts' THEN ${sale_price} ELSE 0 END) /
      NULLIF(SUM(${sale_price}), 0)
    ;;
      value_format_name: percent_1
    }

    measure: weekend_sales_percentage {
      label: "Weekend Sales %"
      type: number
      sql:
      100.0 *
      SUM(CASE WHEN EXTRACT(DAYOFWEEK FROM ${created_raw}) IN (1,7) THEN ${sale_price} ELSE 0 END) /
      NULLIF(SUM(${sale_price}), 0)
    ;;
      value_format_name: percent_1
    }
    ...
    dimension: event_day {
      label: "Event Day"
      description: "Day number since event started (assumes Dec 1 start)"
      type: number
      sql: DATE_DIFF(${created_date}, DATE('2023-12-01'), DAY) + 1 ;;
    }

    dimension: event_week {
      label: "Event Week"
      description: "Week of the book fair event"
      type: string
      case: {
        when: {
          sql: ${event_day} <= 7 ;;
          label: "Week 1 - Grand Opening"
        }
        when: {
          sql: ${event_day} <= 14 ;;
          label: "Week 2 - Building Momentum"
        }
        when: {
          sql: ${event_day} <= 21 ;;
          label: "Week 3 - Peak Sales"
        }
        when: {
          sql: ${event_day} <= 28 ;;
          label: "Week 4 - Final Days"
        }
        else: "Extended Period"
      }
    }

    dimension: shopping_time_category {
      label: "Shopping Time"
      description: "Time of day categorization for book shopping patterns"
      type: string
      case: {
        when: {
          sql: EXTRACT(HOUR FROM ${created_time}) BETWEEN 10 AND 12 ;;
          label: "Morning (10-12)"
        }
        when: {
          sql: EXTRACT(HOUR FROM ${created_time}) BETWEEN 12 AND 15 ;;
          label: "Afternoon (12-15)"
        }
        when: {
          sql: EXTRACT(HOUR FROM ${created_time}) BETWEEN 15 AND 18 ;;
          label: "Late Afternoon (15-18)"
        }
        when: {
          sql: EXTRACT(HOUR FROM ${created_time}) BETWEEN 18 AND 21 ;;
          label: "Evening (18-21)"
        }
        else: "Other Hours"
      }
    ...
    dimension: weekend_vs_weekday {
      label: "Weekend vs Weekday"
      type: string
      case: {
        when: {
          sql: EXTRACT(DAYOFWEEK FROM ${created_raw}) IN (1, 7) ;;
          label: "Weekend"
        }
        else: "Weekday"
      }
